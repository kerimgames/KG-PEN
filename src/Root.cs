...
namespace gInk
{
	...
	public class Root
	{
		public const string AppDisplayName = "kg pen";
		...
		public bool AutoTextCorrection = false;
		...
		public FormTaskbar FormTaskbar;
		...
		public Root()
		{
			...
			FormTaskbar = new FormTaskbar(this);
			SetTrayIconColor();
			FormCollection = null;
			FormDisplay = null;
		}
		...
		public void SetDefaultConfig()
		{
			...
			AutoScroll = false;
			WhiteTrayIcon = false;
			SnapshotBasePath = "%USERPROFILE%/Pictures/kg pen/";
		}
		...
		public void SetTrayIconColor()
		{
			...
			if (WhiteTrayIcon)
			{
				if (File.Exists("icon_kgink.ico"))
					icon = new Icon("icon_kgink.ico");
				else if (File.Exists("icon_white.ico"))
					icon = new Icon("icon_white.ico");
				else
					icon = global::gInk.Properties.Resources.icon_white;
			}
			else
			{
				if (File.Exists("icon_kgink.ico"))
					icon = new Icon("icon_kgink.ico");
				else if (File.Exists("icon_red.ico"))
					icon = new Icon("icon_red.ico");
				else
					icon = global::gInk.Properties.Resources.icon_red;
			}
			trayIcon.Icon = icon;
			if (FormTaskbar != null)
			{
					...
					switch (sName)
					{
						...
						case "CANVAS_CURSOR":
							if (sPara == "0")
								...
								CanvasCursor = 1;
							break;
						case "AUTO_TEXT_CORRECTION":
							if (sPara.ToUpper() == "TRUE" || sPara == "1" || sPara.ToUpper() == "ON")
								AutoTextCorrection = true;
							else
								AutoTextCorrection = false;
							break;
					}
				}
			}
