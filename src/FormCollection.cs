...
namespace gInk
{
	public partial class FormCollection : Form
	{
		...
		private Button btMoreColors;
		private FormColorWheel colorWheelForm;
		...
		private System.Windows.Forms.Timer AutoTextTimer;
		private const int AutoTextDelayMs = 2000;
		...
		private bool InTextCorrection = false;
		...
		public FormCollection(Root root)
		{
			...
			for (int b = 0; b < Root.MaxPenCount; b++)
			{
				...
				string penTip = Root.Local.ButtonNamePen[b] + " (" + Root.Hotkey_Pens[b].ToString() + ")";
				this.toolTip.SetToolTip(this.btPen[b], penTip);
			...
			btMoreColors = new Button();
			btMoreColors.Text = "Renk+";
			btMoreColors.Height = btStop.Height;
			btMoreColors.Width = (int)(btStop.Height * 1.2);
			btMoreColors.Left = cumulatedleft;
			btMoreColors.Top = (int)(gpButtons.Height * 0.07);
			btMoreColors.FlatStyle = FlatStyle.Flat;
			btMoreColors.FlatAppearance.BorderSize = 1;
			btMoreColors.Click += MoreColors_Click;
			btMoreColors.MouseDown += gpButtons_MouseDown;
			btMoreColors.MouseMove += gpButtons_MouseMove;
			btMoreColors.MouseUp += gpButtons_MouseUp;
			this.toolTip.SetToolTip(this.btMoreColors, "Daha fazla renk");
			gpButtons.Controls.Add(btMoreColors);
			cumulatedleft += (int)(btMoreColors.Width * 1.1);
			...
			btOptions.Width = (int)(gpButtons.Height * 1.2);
			btOptions.Height = (int)(gpButtons.Height * 0.88);
			btOptions.Top = (int)(gpButtons.Height * 0.07);
			btOptions.Left = cumulatedleft;
			btOptions.Text = "Seçenekler";
			btOptions.FlatStyle = FlatStyle.Flat;
			btOptions.FlatAppearance.BorderSize = 1;
			btOptions.Click += btOptions_Click;
			btOptions.MouseDown += gpButtons_MouseDown;
			btOptions.MouseMove += gpButtons_MouseMove;
			btOptions.MouseUp += gpButtons_MouseUp;
			gpButtons.Controls.Add(btOptions);
			cumulatedleft += (int)(btOptions.Width * 1.1);

			btAbout = new Button();
			btAbout.Width = (int)(gpButtons.Height * 1.0);
			btAbout.Height = (int)(gpButtons.Height * 0.88);
			btAbout.Top = (int)(gpButtons.Height * 0.07);
			btAbout.Left = cumulatedleft;
			btAbout.Text = "Hakkında";
			btAbout.FlatStyle = FlatStyle.Flat;
			btAbout.FlatAppearance.BorderSize = 1;
			btAbout.Click += btAbout_Click;
			btAbout.MouseDown += gpButtons_MouseDown;
			btAbout.MouseMove += gpButtons_MouseMove;
			btAbout.MouseUp += gpButtons_MouseUp;
			gpButtons.Controls.Add(btAbout);
			cumulatedleft += (int)(btAbout.Width * 1.1);
			cumulatedleft += (int)(btStop.Width * 0.8);
			...
			this.toolTip.SetToolTip(this.btOptions, "Seçenekler");
			this.toolTip.SetToolTip(this.btAbout, "Hakkında");

			AutoTextTimer = new System.Windows.Forms.Timer();
			AutoTextTimer.Interval = AutoTextDelayMs;
			AutoTextTimer.Tick += AutoTextTimer_Tick;
		}

		private void IC_Stroke(object sender, InkCollectorStrokeEventArgs e)
		{
			SaveUndoStrokes();
			if (Root.AutoTextCorrection)
			{
				AutoTextTimer.Stop();
				AutoTextTimer.Start();
			}
		...
		private void AutoTextTimer_Tick(object sender, EventArgs e)
		{
			AutoTextTimer.Stop();
			if (Root.AutoTextCorrection)
				TryAutoTextCorrection();
		}

		private Recognizer GetTurkishRecognizer()
		{
			try
			{
				Recognizers recognizers = new Recognizers();
				return recognizers.GetDefaultRecognizer(1055); // Turkish (tr-TR)
			}
			catch
			{
				// Ignore if recognizer list isn't available.
			}
			return null;
		}

		private void TryAutoTextCorrection()
		{
			if (InTextCorrection)
				return;
			...
			InTextCorrection = true;
			try
			{
				Recognizer trRecognizer = GetTurkishRecognizer();
				if (trRecognizer == null)
					return;

				RecognizerContext rc = trRecognizer.CreateRecognizerContext();
				rc.Strokes = IC.Ink.Strokes;
				RecognitionStatus status;
				RecognitionResult result = rc.Recognize(out status);
				...
				Rectangle inkBounds = IC.Ink.Strokes.GetBoundingBox();
				Point pt1 = new Point(inkBounds.Left, inkBounds.Top);
				Point pt2 = new Point(inkBounds.Right, inkBounds.Bottom);
				IC.Renderer.InkSpaceToPixel(Root.FormDisplay.gOneStrokeCanvus, ref pt1);
				IC.Renderer.InkSpaceToPixel(Root.FormDisplay.gOneStrokeCanvus, ref pt2);
				Rectangle pixelBounds = Rectangle.FromLTRB(
					Math.Min(pt1.X, pt2.X),
					Math.Min(pt1.Y, pt2.Y),
					Math.Max(pt1.X, pt2.X),
					Math.Max(pt1.Y, pt2.Y)
				);
				Color color = IC.DefaultDrawingAttributes.Color;
				float fontSize = Math.Max(12f, pixelBounds.Height);

				Root.TextAnnotations.Add(new TextAnnotation
				{
					Text = text,
					Bounds = pixelBounds,
					Color = color,
			...
			{
				// If no recognizer installed or recognition fails, ignore.
			}
			finally
			{
				InTextCorrection = false;
			}
		}
		...
		private void MoreColors_Click(object sender, EventArgs e)
		{
			...
			int pen = Root.CurrentPen;
			if (pen < 0 || pen >= Root.MaxPenCount)
				pen = Root.LastPen >= 0 ? Root.LastPen : 1;
			Point anchor = btMoreColors.PointToScreen(new Point(0, 0));
			Point pop = new Point(anchor.X, anchor.Y - 210);

			if (colorWheelForm != null && !colorWheelForm.IsDisposed)
				colorWheelForm.Close();

			colorWheelForm = new FormColorWheel(Root.PenAttr[pen].Color, pop);
			colorWheelForm.ColorChanged += (c) =>
			{
				Root.PenAttr[pen].Color = c;
				btPen[pen].BackColor = c;
				btPen[pen].FlatAppearance.MouseDownBackColor = c;
				btPen[pen].FlatAppearance.MouseOverBackColor = c;
				Root.SaveOptions("pens.ini");
				SelectPen(pen);
			};
			colorWheelForm.FormClosed += (s, _e) => { colorWheelForm = null; };
			colorWheelForm.Show(this);
		}
